...
describe('RatingService', () => {
	...
	describe('Adjust Player', () => {
		it('should return adjusted player', async () => {
			...
			const white_result = await service.adjustPlayer(
				player_white,
			...
			const black_result = await service.adjustPlayer(
				player_black,
				quickResult - expected,
				fairness,
				fatigueFactor,
	...
	describe('Adjust Fatigue', () => {
		it('should return adjusted fatigue', async () => {
			...
			const fatigueFactor =
				(1 - (player_white.fatigue[player_black.id] || 0) * 0.4) *
				(1 - (player_black.fatigue[player_white.id] || 0) * 0.4);
			const white_result = await service.updateFatigue(
				player_white,
				player_black.id.toString(),
				fairness * fatigueFactor
			);
			const black_result = await service.updateFatigue(
				player_black,
		...
		it('should return an error if fatigue is a string', async () => {
			...
			const expected = sw / (sw + sb);
			const fairness = expected * (1 - expected);
			const fatigueFactor = (1 - (player_white.fatigue[player_black.id] || 0) * 0.4) * (1 - 0 * 0.4);
			try {
		...
		it('should delete the player fatigue', async () => {
			...
			const fairness = expected * (1 - expected);
			const fatigueFactor = (1 - (player_white.fatigue[player_black.id] || 0) * 0.4) * (1 - 0 * 0.4);
			const result = await service.updateFatigue(
				player_black,
				player_white.id.toString(),
				fairness * fatigueFactor
			);
	...
	describe('Adjust Rating', () => {
		it('should return adjusted rating', async () => {
			...
			const white_result = await service.adjustedRating(
				player_white,
				mockGame.date,
			...
			const black_result = await service.adjustedRating(
				player_black,
				mockGame.date,
				PARTICIPATION_CUTOFF,
				RATING_RETENTION,
