...
export class RatingsController {
	...
	@Get()
	findAll(@Query() query: RatingQuery): Promise<RatingList | DefaultExceptionDto> {
		return this.service.getAll(query);
	}

	@ApiOperation({
		operationId: 'Player Rating',
	...
	@Get('/:name')
	findRating(@Param('name') name: string): Promise<Rating | DefaultExceptionDto> {
		return this.service.getPlayersRating(name);
	}

	// // manually kick off rating generation
	// @Get('/generate')
	// generateRatings(): Promise<void> {
	// 	return this.service.generateRating();
	// }
}
